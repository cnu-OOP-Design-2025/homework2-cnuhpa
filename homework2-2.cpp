...
int findBestStudentInFinal(StudentStruct* students, int numOfStudent) {
    ...
    if(bestfinal > 0) return bestfinalid;
    return 1001;
}
...
int findBestStudent(StudentStruct* students, int numOfStudent) {
    /* TODO */
    float besttotal = 0;
    int besttotalid = 1001;
    for(int i = 0; i < numOfStudent; i++) {
        if(students[i].record.midterm + students[i].record.final > besttotal) {
            besttotal = students[i].record.midterm + students[i].record.final;
            besttotalid = 1001 + i; 
        }
    }
    if(besttotal > 0) return besttotalid;
    return 1001;
}
...
void deleteStudent(StudentStruct* students, int *numOfStudent, int id) {
    ...
    if (idx >= 0) {
        --(*numOfStudent);
        /* TODO */
        students[idx] = StudentStruct("", 0, 0.0f, 0.0f);
    }
}
...
float getMidtermAverage(StudentStruct* students, int numOfStudent) {
    ...
    float avg = 0;
    for(int i = 0; i < numOfStudent; i++) {
        avg += students[i].record.midterm / numOfStudent ;
    }
    if(avg > 0) return avg;
    return 0.0f;
}
...
float getFinalAverage(StudentStruct* students, int numOfStudent) {
    ...
    float avg = 0;
    for(int i = 0; i < numOfStudent; i++) {
        avg += students[i].record.final / numOfStudent ;
    }
    if(avg > 0) return avg;
    return 0.0f;
}
...
float getTotalAverage(StudentStruct* students, int numOfStudent) {
    ...
    float avg = 0;
    for(int i = 0; i < numOfStudent; i++) {
        avg += (students[i].record.midterm + students[i].record.final) / numOfStudent / 2;
    }
    if(avg > 0) return avg;
    return 0.0f;
}
